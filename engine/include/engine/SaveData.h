...
// Persistent player progression. This is the blob payload: six fixed-width
// fields written in declared order. 24 bytes on disk.
struct SaveData {
    ...
    uint32_t unlockFlags = 0; // bitset of unlocked content
    uint32_t bestCombo   = 0; // best combo multiplier reached
};

static_assert(sizeof(SaveData) == 24, "SaveData payload must be 24 bytes on disk");
