...
public:
    ...
    virtual void bindVertexUniform(uint32_t slot, RHIBuffer buffer,
                                   uint64_t offset = 0, uint64_t size = 0) = 0;
    virtual void bindFragmentUniform(uint32_t slot, RHIBuffer buffer,
                                     uint64_t offset = 0, uint64_t size = 0) = 0;
    ...
    // -----------------------------------------------------------------------
    virtual void uploadBuffer(RHIBuffer dst, const void* data,
                              uint64_t size, uint64_t dstOffset = 0) = 0;
    virtual void uploadTexture(RHITexture dst, const void* data, uint64_t size,
                               uint32_t mipLevel = 0, uint32_t layer = 0) = 0;
    virtual void copyBuffer(RHIBuffer dst, uint64_t dstOffset,
                            RHIBuffer src, uint64_t srcOffset, uint64_t size) = 0;
};
